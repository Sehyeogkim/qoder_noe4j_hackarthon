# ARMA implementation contract — Astras

Accepted implementation specification, 2026-09-12. This document defines required behavior; it is not evidence that every stage has run. See [implementation status](docs/implementation-log.md) for completed checks and blockers, and [setup](docs/setup.md) for executable commands. The superseded specification is preserved in [plan-before-astras.md](docs/plan-before-astras.md); its `every_action` cadence and undecided task/GPU choices are superseded by this contract.
...
| Area | Fixed decision |
| --- | --- |
| Working branch | Exactly `Astras`; preserve existing work and prior prototypes in `bin/` |
...
| Compute | One RunPod A40 48 GB, 60 GB working volume, maximum six hours |
...
Use states 0–3 for memory collection, 4 for smoke/calibration, 10–12 for held-out evaluation. Assert all exist. Collect one no-memory attempt per collection state; allow at most two additional retries of failed states in ascending state order. Record all attempts, including failures/unknowns. Freeze eligible successful/failed real collection IDs before evaluation. Evaluation writes never enter that snapshot.

Target three conditions × three held-out states = nine evaluation episodes. If cost prevents this, evaluate the largest affordable prefix with every condition per state. Record interruptions and incomplete groups explicitly. Do not label partial coverage complete.
...
Total cap: $10. Reserve $5 compute/storage, $4 API, $1 shutdown/uncertainty. A40 Secure Cloud was observed at $0.49/hour during planning; recheck live price and availability before creation and do not silently substitute another GPU. One 60 GB working volume; maximum six hours, reduced if costs require it.
...
1. Repository/contracts: `Astras`, preserved prototypes, secret exclusions, schemas, runner, budget ledger, fake worker. Pass: complete mock loop with no duplicate action.
...
5. Collection/demo: real memory, frozen snapshot, held-out comparison and portable export. Pass: instructions trace to actual retrieved evidence; all reported successes have task-predicate support.
