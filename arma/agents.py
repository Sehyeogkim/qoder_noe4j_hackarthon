...
from .contracts import PlannerDecision, RetrievalDecision, EvaluationRecord, SKILL, STAGES, GOAL, validate_evaluation, validate_planner, validate_retrieval
from .budget import Budget

ROOT = Path(__file__).resolve().parent.parent

class GeminiAgents:
    retrieval_input_version = "current-and-historical-before-after-v2"
    ...
    def call(self, role, schema, context, images, validator=None):
        ...
        estimate = Budget.api_cost(bound_input, max_output)
        repair_note = None
        for attempt in range(2):
            ...
            reservation = self.budget.reserve(estimate + (0.002 if repair_note else 0), metadata={"role":role,"model":self.model})
            started=time.monotonic()
            try:
                response = self.client.models.generate_content(model=self.model, contents=call_parts,
                    config=self.types.GenerateContentConfig(system_instruction=prompt,
                        response_mime_type="application/json", response_json_schema=schema_json,
            ...
            if actual_input is not None:
                record["cost_usd"]=Budget.api_cost(actual_input,actual_output)
    ...
    def retrieve(self, context, planner, candidates, observation):
        ...
        if projected:
            ...
            for phase in ('before','after'):
                ...
                if frame_id not in projected[0]['evidence_ids']:
                    continue
...
class FakeAgents:
    ...
    def retrieve(self,context,planner,candidates,observation):
        return RetrievalDecision(**{k:context[k] for k in ("attempt_id","decision_index","based_on_step","observation_id")},
            decision="keep",executed_instruction=GOAL,reason="Synthetic test agent")
    def evaluate(self,context,planner,retrieval,result,outcome):
